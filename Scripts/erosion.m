function result = erosion(sourceimg, selement)
% This function eroses the given source image according to a structuring element
% selement. It simply pads the sourceimg matrix with the extra dimensions
% that structuring element has according to the corner values with 1 and
...
center = [ceil(srow/2), ceil(scol/2)]; % center of the erosion

% row and column number of the source image
imgrow = size(sourceimg, 1);
imgcol = size(sourceimg, 2);

% padarray function adds (center(1)-1) rows and columns with 1's to the
% four sides of the source image
padMatrix = padarray(sourceimg,[center(1)-1, center(2)-1], 1);
